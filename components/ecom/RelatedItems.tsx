...
const RelatedItems = ({ className, relatedItems, }: Props) => {
  return (
    <div className={className}>
      ...
      <div className="mt-12 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-6 lg:gap-x-8">
        {
          relatedItems.map(item => <ItemSquare key={`related-${item.itemId}`} item={item} className="max-w-xs my-8" />)
        }
      </div>
    </div>
  )
